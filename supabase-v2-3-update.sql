-- Frever Wishlist v2.3 schema + fixes
...
using (
  created_by = auth.uid()
  or exists (
    select 1 from public.wishlist_members wm
...
begin
  select id into found_wishlist_id
  ...
  insert into public.wishlist_members (wishlist_id, user_id, can_view, can_edit)
  values (found_wishlist_id, auth.uid(), true, false)
