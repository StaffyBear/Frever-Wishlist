...
let wishlist=null,gifts=[],isOwner=false;
...
async function loadWishlist(){
 ...
 if(error){document.getElementById("giftList").innerHTML=`<div class="empty">${error.message}</div>`;return}
 wishlist=data;isOwner=wishlist.created_by===user.id;
 document.getElementById("title").textContent=`${wishlist.person_name}'s Wishlist`;
 document.getElementById("subtitle").textContent=isOwner?"This is your wishlist. You can add, edit and delete gifts.":"This wishlist was shared with you. You can update gift status.";
...
async function loadViewers(){
 const list=document.getElementById("viewerList");list.innerHTML=`<div class="empty">Loading viewers...</div>`;
 ...
 if(!data||!data.length){list.innerHTML=`<div class="empty">No viewers yet.</div>`;return}
 list.innerHTML=data.map(v=>`<div class="viewer-row"><div><p><strong>${v.display_name||v.email||"Unknown user"}</strong></p><p class="small">${v.email||""}</p></div>${v.user_id===user.id?`<span class="pill">Owner</span>`:`<button class="text-btn danger" data-remove-viewer="${v.member_id}" type="button">Remove</button>`}</div>`).join("");
 document.querySelectorAll("[data-remove-viewer]").forEach(btn=>btn.addEventListener("click",async()=>{if(!confirm("Remove this person's access?"))return;const{error}=await supabase.rpc("remove_wishlist_viewer",{member_input:btn.dataset.removeViewer});if(error)return alert(error.message);await loadViewers()}));
}
async function loadGifts(){
 const{data,error}=await supabase.from("gifts").select("*").eq("wishlist_id",wishlistId).order("created_at",{ascending:false});
 if(error){document.getElementById("giftList").innerHTML=`<div class="empty">${error.message}</div>`;return}
 gifts=data||[];renderGifts();
}
function sortedGifts(){
 const search=document.getElementById("search").value.trim().toLowerCase(),sort=document.getElementById("sort").value;
 let filtered=gifts.filter(g=>!search||(g.name||"").toLowerCase().includes(search)||(g.notes||"").toLowerCase().includes(search));
 filtered.sort((a,b)=>{if(sort==="oldest")return new Date(a.created_at)-new Date(b.created_at);if(sort==="priceLow")return Number(a.cost||0)-Number(b.cost||0);if(sort==="priceHigh")return Number(b.cost||0)-Number(a.cost||0);if(sort==="nameAZ")return(a.name||"").localeCompare(b.name||"");return new Date(b.created_at)-new Date(a.created_at)});
 return filtered;
}
function renderGifts(){
 const list=document.getElementById("giftList"),display=sortedGifts();
 if(!display.length){list.innerHTML=`<div class="empty">${isOwner?"No gifts found. Tap + to add one.":"No gifts found."}</div>`;return}
 list.innerHTML=display.map(g=>`<article class="gift-card"><div class="gift-img">${g.image_url?`<img src="${g.image_url}" alt="">`:"🎁"}</div><div><h3>${g.name}</h3><div class="small">${money(g.cost)} · Qty ${g.quantity||1}</div><div class="actions">${g.gift_url?`<a class="link" href="${g.gift_url}" target="_blank" rel="noopener">Open link</a>`:""}${isOwner?`<button class="text-btn" data-edit="${g.id}">Edit</button><button class="text-btn danger" data-delete="${g.id}">Delete</button>`:""}</div><div class="pills"><span class="pill">${g.priority||"N/A"}</span><span class="pill ${g.status||"Available"}">${g.status||"Available"}</span></div><div class="status-link-row"><button class="text-btn ${(g.status||"Available")==="Available"?"active":""}" data-status-id="${g.id}" data-status="Available">Available</button><button class="text-btn ${(g.status||"Available")==="Reserved"?"active":""}" data-status-id="${g.id}" data-status="Reserved">Reserved</button><button class="text-btn ${(g.status||"Available")==="Purchased"?"active":""}" data-status-id="${g.id}" data-status="Purchased">Purchased</button></div></div></article>`).join("");
...
document.getElementById("search").addEventListener("input",renderGifts);document.getElementById("sort").addEventListener("change",renderGifts);
...
document.getElementById("settingsBtn").addEventListener("click",async()=>{if(isOwner)await loadViewers();openModal("wishlistModal")});
...
document.getElementById("addGiftForm").addEventListener("submit",async event=>{event.preventDefault();const{error}=await supabase.from("gifts").insert({wishlist_id:wishlistId,name:document.getElementById("addGiftName").value.trim(),gift_url:document.getElementById("addGiftUrl").value.trim()||null,image_url:document.getElementById("addGiftImage").value.trim()||null,cost:document.getElementById("addGiftCost").value||null,quantity:Number(document.getElementById("addGiftQty").value||1),priority:document.getElementById("addGiftPriority").value,status:document.getElementById("addGiftStatus").value,notes:document.getElementById("addGiftNotes").value.trim(),created_by:user.id});if(error)return alert(error.message);event.target.reset();document.getElementById("addGiftQty").value=1;document.getElementById("addGiftPriority").value="N/A";document.getElementById("addGiftStatus").value="Available";closeModal("addGiftModal");await loadGifts()});
document.getElementById("editGiftForm").addEventListener("submit",async event=>{event.preventDefault();const id=document.getElementById("editGiftId").value;const{error}=await supabase.from("gifts").update({name:document.getElementById("editGiftName").value.trim(),gift_url:document.getElementById("editGiftUrl").value.trim()||null,image_url:document.getElementById("editGiftImage").value.trim()||null,cost:document.getElementById("editGiftCost").value||null,quantity:Number(document.getElementById("editGiftQty").value||1),priority:document.getElementById("editGiftPriority").value,status:document.getElementById("editGiftStatus").value,notes:document.getElementById("editGiftNotes").value.trim()}).eq("id",id);if(error)return alert(error.message);closeModal("editGiftModal");await loadGifts()});
document.getElementById("wishlistForm").addEventListener("submit",async event=>{event.preventDefault();if(!isOwner)return;const{error}=await supabase.from("wishlists").update({person_name:document.getElementById("editPerson").value.trim(),wishlist_code:document.getElementById("editCode").value.trim().toUpperCase()}).eq("id",wishlistId);if(error)return alert(error.message);closeModal("wishlistModal");await loadWishlist()});
...
document.getElementById("removeSharedWishlistBtn").addEventListener("click",async()=>{if(isOwner)return;if(!confirm("Remove this wishlist from your account?"))return;const{error}=await supabase.rpc("leave_wishlist",{wishlist_input:wishlistId});if(error)return alert(error.message);window.location.href="wishlists.html"});
